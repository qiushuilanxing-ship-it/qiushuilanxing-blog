...
- `docs/10.AI`：Agent、RAG、MCP、Prompt
...
首页保留 vdoing 原有的个人资料、文章分类、标签和站点信息卡片。个人资料与联系方式读取 `themeConfig.ts`；分类、标签和文章数量根据 Markdown Front Matter 自动生成；运行时间、总字数、最近更新和访问量由现有站点统计逻辑动态计算，不在组件中写死数字。

新增文章时请维护 `categories` 和 `tags` 字段，首页侧栏及分类、标签索引页会随内容自动更新。

## GitHub Pages 发布

`deploy.sh` 不写死仓库地址。执行前请把实际博客仓库设置到 `DEPLOY_REPOSITORY`：

```bash
DEPLOY_REPOSITORY=git@github.com:用户名/仓库名.git sh deploy.sh
```
